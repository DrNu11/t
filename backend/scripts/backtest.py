...
sentiment_score 反映 2h 窗口内事件推动价格方向的置信度与幅度预期：

  ±0.05~0.15  弱信号   — 情绪面扰动，无实质资金流变化
  ±0.15~0.35  轻度信号 — 有资金面含义但影响间接
  ±0.35~0.55  中度信号 — 直接推动资产供需或风险偏好
  ±0.55~0.75  强信号   — 直接催化+趋势共振，大概率引发波段
  ±0.75~0.95  极端信号 — 结构性突变/黑天鹅
  ±1.00       绝对确信 — 几乎不使用

叠加规则：
  * direct_catalyst=true → 上浮一档
  * event_strength=low → 上限 ±0.35 | medium → 上限 ±0.70
  * market_confirmation=negative → 下调一档
