...
def test_evaluate_news_l2_passthrough_without_key(monkeypatch):
    import realtime_filter
    monkeypatch.setattr(realtime_filter, "_OPENROUTER_KEY", "")
    r = evaluate_news("某公司发布季度产品公告与市场安排")
    assert r["verdict"] == "L2_PASSTHROUGH"
    assert r["is_noise"] == 0
    assert r["status"] == "PENDING"
    assert r["layer"] == "L2"
